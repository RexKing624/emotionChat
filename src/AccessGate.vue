  ...
  <main v-else class="access-shell">
    <section class="access-card" :class="{'object-card': stage === 'objects'}">
      ...
      <template v-else-if="stage === 'objects'">
        <h1>{{ adding ? t.add : t.choose }}</h1>
        <template v-if="!adding">
          <p class="access-hint">{{ t.chooseHint }}</p>
          <div class="object-list"><button v-for="item in objects" :key="item.id" @click="enter(item.id)"><span class="object-avatar">{{ item.name.slice(0,1).toUpperCase() }}</span><span>{{ item.name }}</span><span class="object-arrow">→</span></button><p v-if="!objects.length" class="access-hint">{{ t.empty }}</p></div>
          <div class="access-actions"><button class="access-secondary" :disabled="busy" @click="openRestore">{{ restoreText.label }}</button><button class="access-secondary" :disabled="busy" @click="openAdd">＋ {{ t.add }}</button><button class="access-secondary" :disabled="busy" @click="lock">{{ t.lock }}</button></div>
        </template>
        <form v-else class="add-object" @submit.prevent="createObject">
          <label>{{ t.name }}<input v-model="chatName" maxlength="40" required :disabled="busy" :placeholder="t.name" /></label>
          <p class="access-hint">{{ t.pickMemory }}</p>
          <div class="memory-options"><button type="button" v-for="item in memoryFolders" :key="item.id" :class="{chosen:newName===item.id}" @click="newName=item.id">{{ item.name }}<span v-if="newName===item.id"> ✓</span></button><p v-if="!memoryFolders.length" class="access-hint">{{ t.empty }}</p></div>
          <div class="access-actions"><button type="button" class="access-secondary" :disabled="busy" @click="adding=false">{{ t.back }}</button><button class="start-chat" :disabled="busy || !newName || !chatName.trim()">{{ t.create }}</button></div>
        </form>
      </template>
      <template v-else-if="stage === 'restore'">
        <h1>{{ restoreText.label }}</h1><p class="access-hint">{{ restoreText.hint }}</p>
        <template v-if="!restoreVerified">
          <div class="pin-dots" role="status" :aria-label="`${restorePin.length} / 6`"><span v-for="n in 6" :key="n" :class="{filled:restorePin.length>=n}"></span></div>
          <div class="pin-keypad"><button v-for="n in 9" :key="n" :disabled="busy" @click="restoreDigit(String(n))">{{ n }}</button><button class="pin-small" :disabled="busy" @click="restorePin='';error=''">{{ t.clear }}</button><button :disabled="busy" @click="restoreDigit('0')">0</button><button class="pin-small" :disabled="busy" :aria-label="t.delete" @click="restorePin=restorePin.slice(0,-1)">⌫</button></div>
        </template>
        <div v-else class="object-list"><button v-for="item in deletedChats" :key="item.id" :disabled="busy" @click="restoreChat(item.id)">{{ item.name }} · {{ restoreText.action }}</button><p v-if="!deletedChats.length">{{ restoreText.empty }}</p></div>
        <button class="access-secondary" @click="showObjects">{{ t.back }}</button>
...
const restorePin=ref(''),restoreVerified=ref(false),deletedChats=ref([]);
const restoreText=computed(()=>({zh:{label:'恢复对话',hint:'输入解锁密码，查看并恢复已删除的对话。',action:'恢复',empty:'没有已删除的对话'},ja:{label:'会話を復元',hint:'解除用パスコードを入力してください。',action:'復元',empty:'削除した会話はありません'},en:{label:'Restore chats',hint:'Enter your unlock passcode to restore deleted chats.',action:'Restore',empty:'No deleted chats'}}[lang.value]||{}));
function openRestore(){restorePin.value='';restoreVerified.value=false;deletedChats.value=[];error.value='';stage.value='restore';}
function restoreDigit(n){if(busy.value||restoreVerified.value||restorePin.value.length>=6)return;error.value='';restorePin.value+=n;if(restorePin.value.length===6)loadDeleted();}
async function loadDeleted(){busy.value=true;try{deletedChats.value=(await request('deleted-chats',{pin:restorePin.value})).objects;restoreVerified.value=true;error.value='';}catch(e){error.value=e.status===429?t.value.later:t.value.wrong;restorePin.value='';}finally{busy.value=false;}}
async function restoreChat(id){busy.value=true;try{await request('restore-chat',{id,pin:restorePin.value});restorePin.value='';await showObjects();}catch(e){error.value=e.status===429?t.value.later:t.value.failed;}finally{busy.value=false;}}
...
async function showObjects(){restorePin.value='';stage.value='objects';selected.value='';selectObject('');error.value='';try{objects.value=(await request('objects')).objects;}catch(e){if(e.status===401)locked();else error.value=t.value.failed;}}
function enter(id){selected.value=id;selectObject(id);stage.value='chat';}
function locked(){restorePin.value='';restoreVerified.value=false;deletedChats.value=[];adding.value=false;newName.value='';stage.value='unlock';objects.value=[];selected.value='';selectObject('');pin.value='';firstPin.value='';error.value='';document.title='EmotionChat';}
...
function key(e){if(stage.value==='restore'&&!restoreVerified.value&&!busy.value&&!e.metaKey&&!e.ctrlKey&&!e.altKey){if(/^\d$/.test(e.key)){e.preventDefault();restoreDigit(e.key);}else if(e.key==='Backspace'){e.preventDefault();restorePin.value=restorePin.value.slice(0,-1);}return;}if(!['setup','unlock'].includes(stage.value)||busy.value||e.metaKey||e.ctrlKey||e.altKey)return;if(/^\d$/.test(e.key)){e.preventDefault();digit(e.key);}else if(e.key==='Backspace'){e.preventDefault();pin.value=pin.value.slice(0,-1);}}
...
<style>
.restore-pin{width:100%;box-sizing:border-box;padding:14px;border:1px solid #ccc;border-radius:12px;text-align:center;font-size:24px;letter-spacing:8px}
...
/* A stable card: only selectable lists scroll, never the whole entry page. */
.access-shell { position:fixed; inset:0; height:100dvh; min-height:0; overflow:hidden; padding:16px; }
.access-card { box-sizing:border-box; width:min(420px,100%); height:min(660px,calc(100dvh - 32px)); max-height:100%; display:flex; flex-direction:column; align-items:stretch; overflow:hidden; padding:24px 28px 16px; }
.access-card > * { flex-shrink:0; }
.access-logo { align-self:center; }
.access-card .access-languages button { width:auto; min-width:0; height:34px; min-height:0; }
.access-card h1 { margin:18px 0 8px; }
.access-card .object-list { flex:1 1 0; min-height:0; max-height:none; overflow-y:auto; overscroll-behavior:contain; align-content:start; margin:14px 0; }
.access-card .object-list button { flex-shrink:0; min-height:56px; }
.access-card .add-object { flex:1 1 0; min-height:0; display:flex; flex-direction:column; overflow:hidden; margin:12px 0 0; }
.add-object > * { flex-shrink:0; }
.add-object .memory-options { flex:1 1 0; min-height:0; max-height:none; overflow-y:auto; overscroll-behavior:contain; align-content:start; margin-bottom:12px; }
.memory-options button { min-height:44px; }
.access-actions { display:flex; justify-content:center; align-items:center; gap:8px; }
.access-actions button { width:auto; min-width:0; min-height:0; height:38px; padding:6px 10px; white-space:nowrap; }
.access-actions .start-chat { flex:1; background:#eee9f8; color:#7255ac; border:0; border-radius:12px; }
.access-error { margin-top:8px; min-height:20px; }
@media(max-height:620px) {
 .access-card { padding:12px 20px; }
 .access-logo { width:28px; height:28px; }
 .access-brand { margin:3px 0 6px; }
 .access-card h1 { margin:10px 0 4px; font-size:20px; }
 .access-card .access-hint { margin:4px 0; font-size:12px; }
 .access-card .pin-dots { margin:12px 0; }
 .access-card .pin-keypad { width:200px; gap:6px 14px; }
}
@media(max-height:430px) {
 .access-card > .access-logo,.access-card > .access-brand { display:none; }
 .access-card .pin-keypad { width:160px; gap:4px 12px; }
 .access-card .pin-keypad button { font-size:18px; }
}

.access-card .pin-keypad { --key-size:64px; width:auto; max-width:none; grid-template-columns:repeat(3,var(--key-size)); grid-auto-rows:var(--key-size); justify-content:center; gap:10px 18px; }
.access-card .pin-keypad button { box-sizing:border-box; width:var(--key-size); height:var(--key-size); min-width:0; min-height:0; padding:0; border-radius:50%; display:grid; place-items:center; line-height:1; }
@media(max-height:620px) { .access-card .pin-keypad { --key-size:48px; gap:8px 18px; } }
@media(max-height:430px) { .access-card .pin-keypad { --key-size:34px; gap:4px 16px; } }
</style>
